...
function ambiguousCoordinates(s) {
  ...
  let getArray = (str) => {
    ...
    for (let i = 1; i < str.length; i++) {
      let item = str.slice(0, i) + '.' + str.slice(i);
      if (check(item)) {
        result.push(item);
      }
    }
  ...
  for (let i = 1; i < numStr.length; i++) {
    let leftNumStr = numStr.slice(0, i);
    let rightNumStr = numStr.slice(i);
    let leftArr = getArray(leftNumStr);
    let rightArr = getArray(rightNumStr);
    // 把两个数组格式化一下，组合起来
