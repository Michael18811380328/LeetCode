...
function twoSum2(nums, target) {
  ...
  for (let i = 0; i < len; i++) {
    ...
    const index = nums.latIndexOf(another);
    if (index > -1 && i !== index) {
      return [i, index];
    }
  }
...
function rotate3(nums, k) {
  ...
  if (len === 1 || k === 0) return;
  const times = k % len;
  const tailArr = nums.splice(-times, times);
...
function trap(height) {
  ...
  if (maxIndexArr.length > 1) {
    ...
    const rain = max * (maxIndexArr[maxIndexArr.length - 1] - maxIndeArr[0] - middleSum);
    sum += rain;
  }
  let leftSum = 0;
  let leftRainSum = 0;
...
function myAnti(str) {
  ...
  const val = parstInt(str.match(reg));
